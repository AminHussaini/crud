...
export default function GoldRateCalculator() {
  ...
  const fetchLiveRate = async () => {
    ...
    try {
      ...
      // USD to PKR rate (approximate, you may want to fetch this live too)
      const usdToPkr = 278; // Current approximate rate
      const ratePerGram = (data.price / 31.1035) * usdToPkr;
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-yellow-50 to-amber-100 p-4">
      <div className="max-w-6xl mx-auto">
        ...
        {/* Summary */}
        <div
          className={`rounded-xl shadow-xl p-6 mt-6 text-white transition-colors duration-300 ${activeTab === 'buying'
              ? 'bg-gradient-to-r from-green-500 to-emerald-600'
              : 'bg-gradient-to-r from-blue-500 to-cyan-600'
            }`}
        >
          <h2 className="text-3xl font-extrabold mb-6">Summary</h2>

          <div className="space-y-6">
            {/* Selected Carat */}
            <div>
              <label className="block text-sm font-medium opacity-90 mb-2">Selected Carat</label>
              <select
                value={selectedCarat}
                onChange={(e) => setSelectedCarat(e.target.value)}
                className="w-full px-4 py-3 rounded-lg font-semibold text-lg bg-white bg-opacity-20 border border-white border-opacity-30 focus:outline-none focus:ring-2 focus:ring-white focus:ring-opacity-50 transition"
              >
                {carats.map((carat) => (
                  <option key={carat.karat} value={carat.karat} className="text-gray-900">
                    {carat.karat}K Gold
                  </option>
                ))}
              </select>
            ...
            <div className="grid md:grid-cols-2 gap-4">
              {/* Rate × Weight */}
              <div className="bg-white bg-opacity-10 rounded-xl p-5 flex flex-col">
                <span className="text-sm opacity-90">Rate ({selectedCarat}K) × Net Weight ({netWeight}g)</span>
              ...
              <div className="bg-white bg-opacity-10 rounded-xl p-5 flex flex-col">
                <span className="text-sm opacity-90">
                  {activeTab === 'buying'
                    ? `Wastage (${wastage}%)`
                    : `Wastage Deduction (${sellingWastageDeduction}%)`}
                </span>
                <span className="text-2xl md:text-3xl font-bold mt-2">
                  {activeTab === 'buying' ? 'Rs. ' : '- Rs. '}
                  {(() => {
                    const baseRate = useLiveRate ? liveRate : parseFloat(manualRate) || 0;
                    const selectedCaratData = carats.find(c => c.karat === parseInt(selectedCarat));
                    const weight = parseFloat(netWeight) || 0;
                    const wastagePercent = parseFloat(activeTab === 'buying' ? wastage : sellingWastageDeduction) || 0;
                    const caratRate = selectedCaratData ? baseRate * selectedCaratData.purity : 0;
                    const subtotal = caratRate * weight;
              ...
              <span className="text-2xl md:text-3xl font-bold mt-2">
                {activeTab === 'buying'
                  ? `Rs. ${(() => {
                    ...
                    const makingAmount = makingType === 'percentage' ? withWastage * (makingVal / 100) : makingVal;
            ...
            {/* Total */}
            <div className="bg-white bg-opacity-20 rounded-xl p-6 border-2 border-white border-opacity-50 flex flex-col">
              <span className="text-sm opacity-90">
                {activeTab === 'buying' ? 'Total Price' : 'You Receive'}
              </span>
              <span className="text-3xl md:text-4xl font-extrabold mt-2">
                Rs. {(() => {
                  const selectedCaratData = carats.find(c => c.karat === parseInt(selectedCarat));
