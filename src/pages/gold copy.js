  ...
  const fetchLiveRate = async () => {
    ...
    try {
      ...
      async function fetchGoldRate() {
        try {
          const apiKey = ""; // replace with your MetalpriceAPI key
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-yellow-50 to-amber-100 p-4">
      <div className="max-w-6xl mx-auto">
        ...
        {/* Summary */}
        <div className={`rounded-lg shadow-lg p-6 mt-6 text-white ${activeTab === 'buying'
          ...
          <div className="space-y-4">
            ...
            <div>
              <div className="text-sm opacity-90 mb-2">Selected Carat</div>
              <select
                value={selectedCarat}
                onChange={(e) => setSelectedCarat(e.target.value)}
                className="w-full px-4 py-3 bg-white bg-opacity-20 border border-white border-opacity-30 rounded text-white font-bold text-lg focus:outline-none focus:ring-2 focus:ring-white focus:ring-opacity-50"
              >
                {carats.map((carat) => (
